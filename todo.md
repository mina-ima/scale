...
- [x] 計測処理で 24fps を維持（描画間引き/`requestAnimationFrame`）。
- [x] 画像処理の WebWorker 検討（必要に応じて）。(形状検出の画像処理が未実装のため、現時点では不要)
- [x] opencv.js はオプション（サイズ影響を評価）。(形状検出の画像処理が未実装のため、現時点では評価時期尚早)

## 14. セキュリティ/プライバシー（P0）

- [x] 画像/映像は端末内処理のみ（送信なし）を確認。
- [x] 解析/広告/追跡 SDK を入れないことを確認。
- [x] パーミッションは最小権限（カメラのみ）。
