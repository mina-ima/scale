...
export async function initHitTestSource(
  ...
  try {
    ...
    if (!hitTestSource) {
      console.warn('Hit test source was undefined.');
      return null;
    }
...
export async function handleWebXRFallback(): Promise<boolean> {
  ...
  try {
    ...
    if (!session) {
      console.warn(
        'WebXR session failed to start, falling back to 2D measurement.'
      );
      return true;
    }
    ...
    console.error(
      'Error during WebXR session setup, falling back to 2D measurement:',
      error
    );
    return true;
  }
