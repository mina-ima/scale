...
const MeasurePage: React.FC = () => {
  ...
  const getInstructionText = useCallback(() => {
    const { isArMode, arError, isPlaneDetected, points3d, selectionMode, calibrationMode, points, isWebXrSupported } = useMeasureStore.getState();
    const { error } = useMeasureStore.getState(); // errorを直接取得
