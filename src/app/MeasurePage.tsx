...
const MeasurePage: React.FC = () => {
  ...
  const getInstructionText = useCallback(() => {
    const { isArMode, isPlaneDetected, points3d, selectionMode, calibrationMode, points, isWebXrSupported } = useMeasureStore.getState();
    const { error, arError } = useMeasureStore.getState(); // errorとarErrorを直接取得
    ...
    return null;
  }, [isArMode, isPlaneDetected, points3d, selectionMode, calibrationMode, points]);
