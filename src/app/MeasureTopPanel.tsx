...
interface MeasureTopPanelProps {
  getInstructionText: (mode: ItemKey) => string | null;
  mode: ItemKey;
}

const MeasureTopPanel: React.FC<MeasureTopPanelProps> = ({ getInstructionText, mode }) => {
  ...
  return (
    <div className="p-4 pointer-events-auto z-30"> {/* z-indexを高く設定 */}
      <div className="bg-white/80 backdrop-blur p-3 rounded shadow max-w-full">
        ...
        <p className="text-orange-600 text-sm mb-2">{getInstructionText(mode)}</p>
        {measurement?.valueMm && (
